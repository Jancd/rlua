## Capability: coroutine-support (NEW)

M7A introduces first-class Lua `thread` values and the Lua 5.1 `coroutine` library so resumable interpreter execution is available for the supported baseline subset without widening native JIT scope.
